...
(defclass body ()
  ((x :accessor x :initarg :x)
   ...
   (vz :accessor vz :initarg :vz)
   (mass :accessor mass :initarg :mass)))

(defmethod initialize-instance :after ((self body) &rest args)
  (declare (ignore args))
  (with-slots (vx vy vz mass) self
	(setf vx (* vx +days-per-year+)
		  vy (* vy +days-per-year+)
		  vz (* vz +days-per-year+)
...
(defun sun ()
  (make-instance 'body :x 0.0 :y 0.0 :z 0.0 :vx 0.0 :vy 0.0 :vz 0.0 :mass 1.0))


(defclass nbody-system ()
  ((bodies :accessor bodies :initform (create-bodies))))
...
(defmethod advance ((self nbody-system) dt)
  (loop for i below (length (bodies self))
		...
		do (loop for j from (+ i 1) below (length (bodies self))
				 ...
				 for d-squared = (+ (* dx dx) (* dy dy) (* dz dz))
				 for distance  = (sqrt d-squared)
				 for mag       = (/ dt (* d-squared distance))
...
(defclass nbody (benchmark) ())

(defmethod inner-benchmark-loop ((self nbody) inner-iterations)
  (loop with system = (make-instance 'nbody-system)
...
(defmethod benchmark ((self nbody))
  (error "Should never be reached"))

(defmethod verify-result ((self nbody) result)
  (declare (ignore result))
  (error "Should never be reached"))


;; (format t "~A~%" (inner-benchmark-loop (make-instance 'nbody) 1))
